...
# Usage: rake post title="A Title" [description="description"] [header_img="img/in_posts/img.png"] [tags=[tag1,tag2]] [category="category"]
desc "Begin a new post in #{CONFIG['post_path']}"
task :post do
  abort("rake aborted: '#{CONFIG['post_path']}' directory not found.") unless FileTest.directory?(CONFIG['post_path'])
  title = ENV["title"] || "new-post"
  description = ENV["description"] || ""
  author = CONFIG['author']
  header_img = ENV["header_img"] || CONFIG['default_header_img']
  tags = ENV["tags"] || "[]"
  ...
  filename = File.join(CONFIG['post_path'], "#{date}-#{slug}.#{CONFIG['post_ext']}")
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "header_img: #{header_img}"
